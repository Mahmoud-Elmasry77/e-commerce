...
function Acc({ setData, setCartnav, setNum, num, setCart, cart, render, setRender,setAddcart }) {
...
if(locaData && locaData.length >=1){  
  ...
  return (
    ...
    <Container>
    <div>
        <h1 className='product-acc-h1'>
        Accessories
        </h1>
        <p className='product-acc-p'>Nam nec tellus a odio tincidunt auctor a ornare odio. Sed non mauris vitae erat consequat auctor eu in elit.
      ...
      <Row>
        ...
        {accProducts && accProducts.map((product, indix) => (
          <Col xs={6} md={4} lg={3} key={`${product.id}-${indix}`}>
                <div  className="link-product-acc">
